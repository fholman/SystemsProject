...
void lineFollowing() {
  IR_Reading(); // retrieves IR values
  ...
  if (rightT == 0 && forward == 0 && leftT == 0) {
    if (sensorValueCentre > highThreshold) { // if buggy is centred
      ...
      if (sensorValueRight > lowThreshold) { //if buggy needs additional correction to the right (should hardly happen)
        ...
        leftMotorSpeed = maxSpeedL - (4000/sensorValueLeft); 
        leftReverseSpeed = 0;
      }
        ...
        rightMotorSpeed = maxSpeedR - (4000/sensorValueRight);
        rightReverseSpeed = 0;
      }
          ...
          // uses this condition if the buggy is currently mapping the paths and comes to the end of a line
            ...
            if (curPathArr[counter] == 0 && collecting) { // if array reaches a 0 this means it has reached the end of the path and is ready to pick up object
              ...
              if (colour == 7) { //checks that it is actually at base
                ...
                if (servoColour == 5 || (redAtBase >= 2 && servoColour == 4)) { // if green
  ...
  else { // dont correct until finished turning - leftT, rightT, and forward variables represent the buggy turning 
         //(so when turning line does not intefer with buggys direction)
...
void mappingRoutes() {
  ...
  if (!(abs(prevPos - curPos) == 2)) { // if buggy has returned back on itself then does not add new potential paths to dfs list
...
void choosePath() {
  if (curPathArr[counter] == 1 || leftT == 1) { // buggy will turn left
    ...
    rightReverseSpeed = 0;
    leftMotorSpeed = 0; 
    leftReverseSpeed = maxSpeedL - 105;
    leftT = 1;
    ...
    rightMotorSpeed = 0;
    rightReverseSpeed = maxSpeedR - 105;
    leftMotorSpeed = maxSpeedL; 
    leftReverseSpeed = 0;
    rightT = 1;
...
// controls buggy motion
void steer(int leftReverse, int leftForward, int rightReverse, int rightForward) {
  analogWrite(MOT_A1_PIN, leftReverse);
  analogWrite(MOT_A2_PIN, leftForward);
  analogWrite(MOT_B1_PIN, rightReverse);
  analogWrite(MOT_B2_PIN, rightForward);
}
...
void checkStateUI(int s) { // checks values returned from bluetooth sensor
  if (s == 9) { // if user would like buggy to return to base
  ...
  else if (s==56) { // manual alert sound
    detection = true;
    sound();
    sound();
    sound();
    ...
    IR_Reading(); // retrieves IR sensor values
...
void pickUpEx() {
  ...
  // reverses buggy to put space between object and door
  ...
  // will spin dc motor until reached limit switch - gripper will move down rail
  ...
  // opens gripper to ready to hold object
  servoPos = 30;
  servo.write(servoPos);
  delay(500);
...
void pickUpRe() { // code to pick up objects
  // moves buggy forwards towards object
  steer(0, maxSpeedL, 0, maxSpeedR);
  delay(300);
  steer(0, 0, 0, 0);
  delay(500);
  ...
  // slowly closes gripper
  for (servoPos = 30; servoPos < 80; servoPos+=2) {
    servo.write(servoPos);
    delay(50);
  }
  ...
  // will move gripper back up rail for 10 seconds
...
void dropOff() {
  ...
  if (servoColour == 4) { // red
    if (redAtBase >= 2) {
      ...
      turnDelay = redAtBase*200 - 200;
      delay(turnDelay);
    }
    else {
      steer(maxSpeedL, 0, 0, maxSpeedR); // rotates slightly to left for first two objects
      turnDelay = 200 - redAtBase*200;
      delay(200 - redAtBase*200);
    }
    steer(0, 0, 0, 0);
    ...
    turnDelay = 1000 - greenAtBase*250;
    delay(turnDelay);
    steer(0, 0, 0, 0);
  ...
  // keeps gripper closed around object
  servoPos = 80;
  ...
  while (switchState == HIGH) { // goes down until switch is hit
    ...
    digitalWrite(in2, HIGH);
    switchState = LIMIT_SWITCH_END.getState();
    servo.write(servoPos); // ensures gripper is holding object securing while dropping off object
  }
  
  digitalWrite(in1, LOW); // turns off motor
  digitalWrite(in2, LOW);

  // opens up servo to drop off object
  servoPos = 30;
  servo.write(servoPos);
  delay(500);

  // reverses buggy away from object
  steer(maxSpeedL, 0, maxSpeedR, 0);
  delay(650);
  steer(0, 0, 0, 0);
  delay(500);
  ...
  // buggy no longer holding object
  holdingObj = false;

  // after all objects have been placed at base buggy will turn back onto base
  if ((servoColour == 5 && pathNumber == conversePaths.size()-1) || findingBase) {
    steer(maxSpeedL, 0, 0, maxSpeedR);
    delay(turnDelay/3*2);
  }
  else if ((servoColour == 6 && pathNumber == conversePaths.size()-1) || findingBase) {
    steer(0, maxSpeedL, maxSpeedR, 0); 
    delay(turnDelay);
  }
...
void sound() {
  int numNotes;
  if (detection) { // sets number of notes for detection sound
    numNotes = 2;
  }
  else { // sets number of notes for mapping sound
    numNotes = 8;
  }
  for (int thisNote = 0; thisNote < numNotes; thisNote++) {
    ...
    int noteDuration;
    if (detection) { // plays detection sound
      noteDuration = 1000/noteDurations[thisNote];
      tone(10, melody[thisNote],noteDuration);
    }
    else { // plays mapping sound
...
void loop() {
  ...
  if (loopCounter%200==0 && !atBase) { // will only check for distance every 200 iterations of the loop otherwise there was too much delay from the sensor
    if (holdingObj) { // locks servo position every 200 iterations of the loop - saves on power by not locking every single iteration
    ...
    if (!atBase) {
      ...
      long dis = calcDist(); // calculates distance from ultrasonic sensor
   
      while (dis < 10 && leftT == 0 && rightT == 0) {
        if (detection == false) {
        ...
        if(loopCounter==4 && !mapping && !endPath && collecting) { // if been stopped for 1500 iterations buggy will turn around and return to base
          Serial.write('D'); // returning to base message to UI
          endPath = true;
          counter = toBase.size()-1;

          // turns around to return to base
          rightMotorSpeed = maxSpeedR;
          rightReverseSpeed = 0;
          leftMotorSpeed = 0; 
          leftReverseSpeed = maxSpeedL;
          ...
          for (int j=0; j<toBase.size(); j++) {
            curPathArr[j]=toBase.get(j); // saves path to return to base to the path array
          }
          conversePaths.add(conversePaths.get(pathNumber)); // sets current failed path back onto the list to try for the object again
        }
        dis = calcDist();
      }
