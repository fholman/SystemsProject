    ...
    if (sensorValueCentre > lowThreshold) { //if buggy is centred
      ...
      if (sensorValueRight > lowThreshold) { //if buggy needs additional correction to the right - should hardly happen
        ...
        if (mapping) {
          ...
          tcs.getRGB(&rrrr, &gggg, &bbbb);
          //colourrrrr = detectColour(rrrr, gggg, bbbb);

          
          path.add(dfs.get(0)*10+4);
          //Serial.write(colourrrrr);
...
//          }
          ...
          // i think this if statement is incorrect
          // I think I am making mapping = false too soon
          // Or need a different function and new variables
...
void choosePath() {
  ...
  if (curPathArr[counter] == 1 || leftT == 1 || initial) { // using initial here should hopefully fix its error, I think buggy is making an error here
    rightMotorSpeed = 150;
    rightReverseSpeed = 0;
    leftMotorSpeed = 0; 
    leftReverseSpeed = 0;
