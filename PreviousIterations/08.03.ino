...
boolean completed = false; // turns true when buggy has traversed every path in the path list
...
boolean manualMode = false; // checks if buggy is in manual mode
...
void setup() {
  ...
  */
  ...
  steer(0, 0, 0, 0);
  completed = true;
  atBase = false; // start not at base - as once program starts will use colour readings to check for base
}
  ...
  if (rightT == 0 && forward == 0 && leftT == 0) {
    if (sensorValueCentre > lowThreshold) { // if buggy is centred
      ...
      if (sensorValueRight > lowThreshold) { //if buggy needs additional correction to the right (should hardly happen)
        ...
        if (finishMap) {
          // code here for dropping off objects
  ...
  */
  ...
  if (loopCounter%200==0 && !atBase) {
    ...
    while (dis < 15 && leftT == 0) {
      ...
      else if(disT==1500 && !mapping && !endPath && collecting) {
        endPath = true;
