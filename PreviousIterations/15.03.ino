...
void movement() {
  ...
  if (rightT == 0 && forward == 0 && leftT == 0) {
    if (sensorValueCentre > lowThreshold) { // if buggy is centred
      ...
      if (sensorValueRight > lowThreshold && sensorValueLeft > lowThreshold && sensorValueCentre > lowThreshold) {
        if (mapping) {
      ...
      if (sensorValueRight > lowThreshold) { //if buggy needs additional correction to the right (should hardly happen)
        ...
        // uses this condition if the buggy is currently mapping the paths and comes to the end of a line
          ...
          if (colour == 7) {
            ...
            rightMotorSpeed = maxSpeedR - 20;
            rightReverseSpeed = 0;
            leftMotorSpeed = 0; 
            leftReverseSpeed = maxSpeedL - 20;
    ...
    if (sensorValueLeft > highThreshold && sensorValueCentre < highThreshold && sensorValueRight < lowThreshold) {
      leftT = 0;
      forward = 0;
    }
