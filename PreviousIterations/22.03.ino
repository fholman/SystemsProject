...
int servoColour; // variable to store colour of object held by buggy
...
int maxSpeedL = 240; // maximum speed of buggy
int maxSpeedR = 200;
...
void movement() {
  ...
  if (rightT == 0 && forward == 0 && leftT == 0) {
    if (sensorValueCentre > highThreshold) { // if buggy is centred
      ...
      leftMotorSpeed = maxSpeedL - (4000/sensorValueRight) + 30; 
      rightMotorSpeed = maxSpeedR - (4000/sensorValueLeft) + 30;
      leftReverseSpeed = 0;
      rightReverseSpeed = 0;
      ...
      if (sensorValueRight > lowThreshold) { //if buggy needs additional correction to the right (should hardly happen)
        ...
        // uses this condition if the buggy is currently mapping the paths and comes to the end of a line
          ...
          if (colour == 7) {
            ...
            rightMotorSpeed = maxSpeedR - 35;
            rightReverseSpeed = 0;
            leftMotorSpeed = 0; 
            leftReverseSpeed = maxSpeedL - 35;
    ...
    if (sensorValueLeft > highThreshold && sensorValueCentre < highThreshold && sensorValueRight < highThreshold) {
      leftT = 0;
      forward = 0;
    }
    else if (sensorValueLeft < highThreshold && sensorValueCentre > highThreshold && sensorValueRight < highThreshold) {
      forward = 0;
    }
    else if (sensorValueLeft < highThreshold && sensorValueCentre < highThreshold && sensorValueRight > highThreshold) {
      rightT = 0;
...
void pickUp() { // code to pick up objects
  ...
  // will move motor back up for 13 seconds
  ...
  Serial.write('E'); // picking up object message to UI
...
void loop() {
  ...
  */
  ...
  /*
  if (loopCounter%300==0 && !atBase) { // will only check for distance every 200 iterations of the loop otherwise there was too much delay from the sensor
    ...
    if (!atBase) {
      ...
      if (dis < 30) { // will slow down speed until it reaches object
        maxSpeedL = 200 - 400/dis;
        maxSpeedR = 160 - 400/dis;
